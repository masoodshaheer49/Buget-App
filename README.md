...
|---|---|---|
| **Fixed bill** | Rent, insurance, a phone plan | A set amount on a **due day each month**, or on a **cycle of N days** for things like a 28-day prepay plan. Shows on the Bills tab and warns you when it's overdue or the pocket is short. |
...
A 28-day phone plan is not monthly. It drifts earlier through the year and bills
thirteen times, not twelve — €260 rather than €240 — so a due-day-of-the-month
field quietly under-budgets it and shows the wrong date.

A bill pocket can therefore take **every N days from a start date** instead of a
day of the month. Occurrences are generated by stepping whole days from the
start (so daylight saving cannot shift one), never before the start date, and a
month can legitimately hold two of them.

Each occurrence is keyed by its own date (`2026-10-30`) where a monthly bill is
keyed by its month (`2026-10`). Both go in the same `billPeriod` field, so every
record written before cycles existed stays valid, and paying one occurrence
leaves its neighbour outstanding.
